...
export function GradingPageContent({ courseId, assignmentId, studentId, assignmentListHref, isAssistant = false }: GradingPageProps) {
    ...
    const [explorerSection, setExplorerSection] = useState<'files' | 'students'>('files');
    ...
    const gradingStudents = useMemo(() => {
        ...
        allSubs.forEach((sub) => {
            const existing = byStudent.get(sub.student_id);
            const incomingTime = new Date(sub.submitted_at).getTime();
            const existingTime = existing ? new Date(existing.latest_submitted_at).getTime() : -1;

            if (!existing) {
                byStudent.set(sub.student_id, {
                    id: sub.student_id,
                    full_name: sub.student?.full_name || `Student #${sub.student_id}`,
                    email: sub.student?.email || '',
                    student_id: sub.student?.student_id,
                    latest_submitted_at: sub.submitted_at,
                    latest_attempt_number: sub.attempt_number,
                    latest_status: sub.status,
                    latest_score: sub.final_score,
                    submission_count: 1,
                    flagged: Boolean(sub.plagiarism_flagged || sub.ai_flagged),
                });
                return;
            }

            existing.submission_count += 1;
            existing.flagged = existing.flagged || Boolean(sub.plagiarism_flagged || sub.ai_flagged);

            if (incomingTime > existingTime) {
                existing.latest_submitted_at = sub.submitted_at;
                existing.latest_attempt_number = sub.attempt_number;
                existing.latest_status = sub.status;
                existing.latest_score = sub.final_score;
            }
        });
    ...
    const navigateToStudent = useCallback((nextStudentId: number) => {
        ...
        setFileContents({});
        setRunResult(null);
        router.push(`${assignmentListHref}/grade/${nextStudentId}`);
    }, [assignmentListHref, router, studentId]);
            ...
            <div className="flex flex-1 min-h-0">
                ...
                <div className="w-12 bg-[#333333] border-r border-[#3c3c3c] flex flex-col items-center py-2 gap-1 shrink-0">
                    <button
                        ...
                        title="Files"
                        className={`w-10 h-10 flex items-center justify-center rounded hover:bg-[#505050] ${explorerOpen && explorerSection === 'files' ? 'text-white border-l-2 border-white' : 'text-[#858585]'}`}
                    >
                    ...
                    <button
                        onClick={() => {
                            setExplorerOpen(true);
                            setExplorerSection('students');
                        }}
                        title="Students"
                        className={`w-10 h-10 flex items-center justify-center rounded hover:bg-[#505050] ${explorerOpen && explorerSection === 'students' ? 'text-white border-l-2 border-white' : 'text-[#858585]'}`}
                    >
                ...
                {explorerOpen && selectedSub && (
                    <div className="w-56 bg-[#252526] border-r border-[#3c3c3c] flex flex-col min-h-0 shrink-0">
                        <div className="px-4 py-2 text-[11px] font-semibold uppercase tracking-wider text-[#bbbbbb]">
                            {explorerSection === 'students' ? 'Students' : 'Submitted Files'}
                        </div>
                        {explorerSection === 'students' ? (
                                ...
                                {gradingStudents.map((s) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={isCurrent}
                                            className={`w-full text-left rounded px-2.5 py-2 border transition-colors ${isCurrent
                                                ? 'bg-[#862733]/30 border-[#862733]/50 text-white cursor-default'
                                                : 'bg-[#1e1e1e] border-[#3c3c3c] text-[#cccccc] hover:bg-[#2a2d2e] hover:border-[#505050]'
                                                }`}
                                        >
                                            <div className="flex items-center gap-2">
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-semibold ${isCurrent ? 'bg-[#862733]/50 text-white' : 'bg-[#3c3c3c] text-[#d4d4d4]'}`}>
                                                    {s.full_name.charAt(0).toUpperCase()}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[11px] font-medium truncate">{s.full_name}</p>
                                                    <p className="text-[10px] text-[#858585] truncate">{s.email || 'No email'}</p>
                                                </div>
                                                {s.flagged && <AlertTriangle className="w-3.5 h-3.5 text-[#f44747] shrink-0" />}
                                            </div>
                                            <div className="mt-1.5 flex items-center justify-between text-[9px] text-[#9fa4aa]">
                                                <span>{s.submission_count} attempt{s.submission_count > 1 ? 's' : ''}</span>
                                                <span className="capitalize">{s.latest_status || 'pending'}</span>
                                            </div>
                                        </button>
                                    );
                                })}
